...
const knowledgeService = new KnowledgeService(path.join(__dirname, '..', '..', '..', 'data'));
export function setupIpcHandlers(ipcMain) {
    // Handle chat messages
    ipcMain.handle(IPC_CHANNELS.CHAT_SEND, async (_event, request) => {
        // Validate input
    ...
    ipcMain.handle(IPC_CHANNELS.CHAT_STREAM, async (event, request) => {
        if (!request?.content || typeof request.content !== 'string') {
            throw new Error('Invalid message content');
        }
        ...
        try {
            ...
            const context = relevantDocs.length > 0
                ...
                : '';
            // Stream response
            await copilotService.streamChat(request.content, context, (chunk, done) => {
    ...
    ipcMain.handle(IPC_CHANNELS.KNOWLEDGE_SEARCH, async (_, query) => {
    ...
    ipcMain.handle(IPC_CHANNELS.KNOWLEDGE_LOAD, async (_, filePath) => {
